...
int levenshteinDistance(const string &s1, const string &s2) {
    ...
    for (size_t i = 1; i <= len1; ++i) {
        for (size_t j = 1; j <= len2; ++j) {
            int cost = (s1[i-1] == s2[j-1]) ? 0 : 1; // проверка одинакоыли буквы слева и сверху
            d[i][j] = min({ d[i-1][j] + 1,      // удаление буквы из слова
                            d[i][j-1] + 1,      // вставка // выбираем что проще удалить встаить или заменить
                            d[i-1][j-1] + cost  // замена буквы возможно
                               });
        }
    }
    return d[len1][len2];
